...
function loadAnimation(data){
    ...
    a_divid.style.width = a_width+'px'
    a_divid.style.height = a_height+'px'
...
function playAnimation(pos,start,limit = 0,reverse = false,callBack = null,framerate = 50){
    if(start!=null){
        animations[pos].f = (start-1);
        animations[pos].init = (start-1);
    }
    animations[pos].status = 'playing'
    animations[pos].loop = false
    if(limit==0){
        //loop es false por defecto
        if(reverse){
            animations[pos].limit = 0;
        }else{
            animations[pos].limit = (animations[pos].frames_total-1)
        }
    }else if(limit==-1){
        //limite es el final por defecto
        //loop ponerlo en true
        if(reverse){
            animations[pos].limit = 0;
        }else{
            animations[pos].limit = (animations[pos].frames_total-1)
        }
        animations[pos].loop = true
    }else{
        //limite es lo que diga el usuario
        animations[pos].limit = (limit-1);
    }
    animations[pos].animacion = setInterval(function(){
        ...
        if(reverse){
            if(animations[pos].f==animations[pos].limit){
                if(animations[pos].loop){
            ...
            if(animations[pos].f==animations[pos].limit){
                if(animations[pos].loop){
                    animations[pos].f = animations[pos].init
                }else{
                    animations[pos].f = (animations[pos].frames.length-1)
                    stopAnimation(pos)
...
function setAnimationFrame(pos,f){
    animations[pos].ctx.clearRect(0,0,animations[pos].w,animations[pos].h)    
    animations[pos].ctx.drawImage(animations[pos].frames[f-1].img,0,0,animations[pos].w,animations[pos].h)
}